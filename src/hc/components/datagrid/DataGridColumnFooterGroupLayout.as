...
{
	import hc.components.datagrid.itemrenderer.DataGridFooterRenderer;
	 ...
	 *  
	 */
	public class DataGridColumnFooterGroupLayout extends LayoutBase
	{
		/**
		 *  Constructor. 
		 *  
		 ...
		 */    
		public function DataGridColumnFooterGroupLayout()
		{
			super();
		}
		...
		override public function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			for (var index:int = 0; /* termination conditions below */; index++)
			{
				...
				if (!renderer)
				{
					var factory:IFactory = null;//column.headerRenderer;  陈顺华，不让从column取headerRenderer，否则会出现checkbox
					if (!factory)
		...
		public function getHeaderRendererAt(columnIndex:int):IGridItemRenderer
		{
			...
			var factory:IFactory = null;//column.headerRenderer; 陈顺华
			if (!factory)
				factory = columnHeaderGroup.headerRenderer;
...
//			renderer.label = column.headerText;
			if(column is DataGridColumn){
				var dgColumn:DataGridColumn = column as DataGridColumn;
				if(dgColumn.summaryFunction != null){	
					var _sumNum:Number = dgColumn.summaryFunction(column.grid.dataGrid.dataProvider,column.dataField);
					if(dgColumn.formatThousandth){
						renderer.label = NumberUtil.formatThousandth(_sumNum);
					}
					else{
						renderer.label = _sumNum + '';
					}
				}else{
					renderer.label = dgColumn.footerText;
				}
			}
			
			const columnHeaderGroup:GridColumnHeaderGroup = columnHeaderGroup;
			
			const dataGrid:spark.components.DataGrid = columnHeaderGroup.dataGrid;
			if (dataGrid)
