...
//		}
		 ...
		 *
		 */
		private function toggleParents(item:Object, tree:mx.controls.Tree, state:int):void
		{
			if (item == null)
				return ;
			else
		 ...
		 *
		 */
		private function toggleChildren(item:Object, tree:mx.controls.Tree, state:int):void
		{
			if (item == null)
				return ;
			else
		 ...
		 *
		 */
		private function getState(tree:mx.controls.Tree, parent:Object):int
		{
			var noChecks:int=0;
			var noCats:int=0;
			...
			if (parent != null)
			{
				...
				while(!cursor.afterLast)
				{
					if (cursor.current[myTree.checkBoxStateField] == myTree.STATE_CHECKED)
						noChecks++;
					else if (cursor.current[myTree.checkBoxStateField] == myTree.STATE_UNCHECKED)
						noUnChecks++;
					else
						noCats++;
					cursor.moveNext();
			...
			if ((noChecks > 0 && noUnChecks > 0) || noCats > 0)
				return myTree.STATE_SCHRODINGER;
			else if (noChecks > 0)
				return myTree.STATE_CHECKED;
			else
				return myTree.STATE_UNCHECKED;
		}
		/**
		 * //TODO:设置项目的父项状态和子项状态
		 * @param event 事件
		...
		private function checkBoxToggleHandler(event:MouseEvent):void
		{
			if (data)
			{
				...
				if (toggle)
				{
					toggleChildren(data, myTree, myTree.STATE_CHECKED);
					//选择项时是否展开子项
				...
				{
					toggleChildren(data, myTree, myTree.STATE_UNCHECKED);
					//取消选择是否收回子项
				...
				myTree.dispatchEvent(new CustomEvent("checkBoxChanged",data));
				//myTree.checkBoxChangeHandleCallback(event,myCheckBox,data,myTree);
			}
		...
		{
			if (state == myTree.STATE_CHECKED)
				checkBox.selected=true;
			else if (state == myTree.STATE_UNCHECKED)
				checkBox.selected=false;
			else if (state == myTree.STATE_SCHRODINGER)
				checkBox.selected=true;
		}
...
//				setCheckState(myCheckBox, data, data[myTree.checkBoxStateField]);
				if (data[myTree.checkBoxStateField] == myTree.STATE_SCHRODINGER)
				{
//					fillCheckBox(true);
					myCheckBox.alpha = 0.4;
				}
				else{
//					fillCheckBox(false);
					myCheckBox.alpha = 1;
